...
export function selectStep(step_id: number, args: string[]) {
	...
	switch (step_id) {
		case 0: { // []
			...
			aY = getCoords(elem).y + elem.offsetHeight + 30;
			text = 'Hi! Gleich kann es mit dem Streamen losgehen.';
			break;
		}
		case 1: { // [ valid?, content ]
			aY = getCoords(document.getElementById('email')).y + 46;
			text = 'Vielleicht sende ich dir mal eine Postkarte!';
			if (!args[0]) text = !args[1] ? 'Wie können wir dich erreichen?' : 'Ich weiß nicht, wie ich dich so erreichen soll.';
			selectGesture(args[0] ? 1 : 2);
			break;
		}
		case 2: { // [ valid?, content ]
			aY = getCoords(document.getElementById('username')).y + 46;
			text = insertBold('Super, ', args[1], ' gefällt mir!');
			if (!args[0]) text = !args[1] ? 'Wie sollen wir dich nennen?' : 'Sorry, so einen langen Namen kann ich mir nicht merken.';
			selectGesture(args[0] ? 1 : 2);
			break;
		}
		case 3: { // [ valid?, content, suggestion ]
			aY = getCoords(document.getElementById('psw')).y + 46;
			text = 'Stärker als ein Braunbär!';
			selectGesture(args[0] ? 0 : 2);
			if (!args[0]) text = !args[1] ? 'Das bleibt zwischen uns ;)' : 'Das ist etwas kurz, meinst du nicht auch?';
		...
		case 4: { // [ valid?, content ]
			...
			text = 'Passt so!';
			if (!args[0]) text = !args[1] ? 'Nochmal! Nochmal!' : 'Das sieht irgendwie falsch aus...';
			if (args[0] && !args[1]) text = 'Erstmal sollten wir uns ein Passwort überlegen...';
			selectGesture(args[0] && args[1] ? 1 : 2);
			break;
		}
		case 5: { // [ missing field ]
			aY = getCoords(document.getElementById('submit_btn')).y + 50;
			text = insertBold('Könntest du dir nochmal das ', args[0] + '-Feld', ' anschauen?');
			selectGesture(2);
			break;
		}
		default:
